...
#define MY_OTA_FIRMWARE_FEATURE<br>#define MY_OTA_FLASH_JDECID 0x2020 | Define OTA feature. OTA stands for “Over The Air firmware updates”. If your node does not utilize Sleep mode you can send<br> new “firmware” (compiled sketch binary) by air.<br> **Here is the link on how to do it.**For OTA we use JDEC Flash chip where<br> the node stores new firmware and once it received and control<br> sum (CRC) is correct it reboots and flashes your new code into<br> the node controller. So we define it is "erase type"<br> as 0x2020 here. 
